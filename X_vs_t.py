...
def plot_field_peak_vs_t(path, code_name='osiris', field_name='e3', fld_slice=None, if_abs=False, use_num_list=False, start=0, count=0, stride=1, t_offset = 0., h_ax=None, field_abs=True, **kwargs):
    '''
    
    '''
    t_array = list()
    peak_array = list()
    outfile_object = outfile.OutFile(code_name=code_name, path=path, field_name=field_name, fld_slice=fld_slice, use_num_list=use_num_list)
...
if __name__ == '__main__':
    ...
    plot_field_peak_vs_t(code_name='osiris', path='/public3/home/sc55489/jobs/2', field_name='e3', fld_slice=3, if_abs=True, use_num_list=True, count=999, color='r', label='')
    plt.xlabel('$t$')
    plt.ylabel('$E_L$')
    plt.legend()
    plt.tight_layout()

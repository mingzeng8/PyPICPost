...
def plot_beam_parameters_vs_t(path, species_name, n0_per_cc, code_name='osiris', gamma_threshold=1., start=0, count=0, stride=1):
    ...
    for i in range(start, start+count*stride, stride):
        ...
        try:
            ...
            print('Warning: error found for file \'{0}\'. Exception message:\n{1}'.format(os_file.path_filename, err))
            continue
        t_array.append(t)
    ...
    h_ax.plot(t_array, np.divide(FWHM_gamma_array, center_gamma_array))
    plt.xlabel('t [$\\omega_p$]')
...
if __name__ == '__main__':
    #plot_beam_parameters_vs_t(code_name='osiris', path='/home/zming/mnt/os_beam3D63', species_name='He_e', n0_per_cc=4.9e16, gamma_threshold=1., start=20, count=999, stride=1)
    plot_beam_parameters_vs_t(code_name='hipace', path='/home/zming/simulations/os2D/Hi_beam3D63', species_name='trailer', n0_per_cc=4.9e16, gamma_threshold=1., start=0, count=999, stride=10)
